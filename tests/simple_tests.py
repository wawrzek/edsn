...
command1 = subprocess.run([sys.executable, script_path, '-v'], capture_output=True, check=True, text=True)
errors += compare_hashes('v1688aquilae-r15.3d.dat')
logfile.write(command1.stdout)
logfile.flush()
print ("Test script with system=Sol, radius=20")
command2 = subprocess.run([sys.executable, script_path, '-v', '-s', 'Sol', '-r', '20'], capture_output=True, check=True, text=True)
